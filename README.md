...
기존 `/docs/project/*`, `/docs/aboutMe/PARK%20EUNDO`, `/blog/dev-story/*` 주소의 접속 호환성을 유지합니다. 내부 페이지 링크와 리디렉션 목적지는 영문 소문자를 사용하고 `/docs/project/`처럼 끝에 `/`를 붙입니다. Netlify의 실제 제공 주소와 맞춰 추가 301 이동을 피하며, 빌드에서 링크·메타데이터·피드·사이트맵의 주소를 검사합니다. 파일 주소와 피드의 영구 식별자는 바꾸지 않습니다.
...
Today는 서버의 `Asia/Seoul` 날짜(00:00~24:00) 기준이며, 동시 접속자나 페이지 조회 수가 아닙니다. 같은 브라우저는 하루 한 번 집계합니다. Total은 이 일별 방문의 누적값으로, 전체 기간의 고유 인원 수는 아닙니다. 기기·브라우저 변경이나 저장소 초기화는 별도 방문으로 잡힐 수 있습니다.

첫 페이지 로드, 탭 복귀, 네트워크 복구 시 조회합니다. 푸터 통계가 화면에 보이고 탭이 활성 상태일 때만 30초마다 추가 조회하며, GET은 방문 수를 늘리지 않습니다. 브라우저 시계 대신 응답의 서울 날짜를 사용합니다. `updatedAt`은 마지막 방문 집계 시각, `refreshedAt`은 이번 응답의 조회 시각입니다.

브라우저의 임의 UUID로 재전송을 구분하며 IP나 이름을 저장하지 않습니다. 서버는 당일 UUID 목록만 유지하고 조건부 쓰기로 동시 업데이트를 처리합니다. 기존 `eundo-site-pulse/stats`의 누적값은 유지하며, 저장소 오류에는 0을 반환하거나 덮어쓰지 않고 503을 반환합니다. 이전 클라이언트의 식별자 없는 POST도 호환되지만, 서버 중복 방지는 새 클라이언트 요청에 적용됩니다.
